...
TIMEOUT /T 1 >NUL
CLS
reg export HKLM\SYSTEM\CurrentControlSet\Services\SharedAccess\Parameters\FirewallPolicy\DomainProfile %homedrive%\TEMPBBDVD\regedit.reg /y
cls
timeout /t 3 >nul
cls
...
echo 2. SSH
echo 3. TelNet
echo 4. SNMP (Only if Specified)
echo 5. LDAP
echo 6. RDP
echo 7. RPC
echo 8. All of the Above
echo.
echo 9. Unblock Specific Ports
echo.
echo 10. Exit Window
echo.
echo.
set /p menu="Please Select the Port You Would Like to Block: (1-10)"
echo.
echo.
if %menu%==1 goto FTP
if %menu%==2 goto SSH
...
if %menu%==7 goto RPC
if %menu%==8 goto All
if %menu%==9 goto UnblockMenu
if %menu%==10 goto Done



:FTP

netsh advfirewall firewall add rule name="Block FTP" protocol=TCP dir=out remoteport=20-21 action=block
netsh advfirewall firewall add rule name="Block FTP" protocol=TCP dir=in remoteport=20-21 action=block
echo.
...
netsh advfirewall firewall add rule name="Block RPC" protocol=UDP dir=out remoteport=135 action=block

netsh advfirewall firewall add rule name="Block RDP" protocol=UDP dir=in remoteport=135 action=block
...
if %menuA%==7 goto RPC0
if %menuA%==8 goto All0
if %menuA%==9 goto Menu
if %menuA%==10 goto Done
...
netsh advfirewall firewall add rule name="Unblock RPC" protocol=UDP dir=out remoteport=135 action=allow

netsh advfirewall firewall add rule name="Unblock " protocol=UDP dir=in remoteport=135 action=allow
echo.
ECHO RPC Port Unblocked
timeout /t 3 >nul
goto UnblockMenu
